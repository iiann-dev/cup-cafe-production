          ...
          <div className="lg:sticky lg:top-32 flex lg:flex-col gap-4 overflow-x-auto pb-4 lg:pb-0">
            {MENU_CATEGORIES.map((cat) => (
              <button
                ...
                className={`font-label-sm text-label-sm px-5 py-3 rounded-full whitespace-nowrap transition-all duration-200 ${
                  activeCategory === cat.id
                    ? 'bg-secondary text-on-secondary shadow-lg'
                    : 'hover:bg-surface-container-low text-on-surface-variant active:bg-surface-container-low'
                }`}
              >
                {cat.label}
              </button>
              ...
              {cat.id === 'design-your-own' ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                  {cat.items.map((item, i) => (
                    <div
                      key={i}
                      className="border-b border-outline-variant/20 pb-4 group animate-[fadeIn_0.3s_ease-out_both]"
                      style={{ animationDelay: `${i * 30}ms` }}
                    >
                      <div className="flex justify-between items-start mb-1">
                        <h4 className="font-headline-md text-[20px] text-primary group-hover:text-secondary transition-colors duration-200">{item.name}</h4>
